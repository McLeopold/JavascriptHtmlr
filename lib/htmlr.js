var Htmlr = (function () {
  ...
  if (new HtmlrString('test').toString() !== 'test') {
    HtmlrString.prototype.toString = HtmlrString.prototype.valueOf = function () {
  ...
  Htmlr.prototype.render_nodes = function (e, data) {
  ...
  var extract = HtmlrExtract.creator();
  ...
  HtmlrEach.prototype.render = function (data) {
    ...
    if (Object.prototype.toString.call(iter) === '[object Array]') {
      for (var i = 0, len = iter.length; i < len; ++i) {
        var item = iter[i];
        this.render_nodes(f, item);
      }
    } else {
      for (var key in iter) { if (iter.hasOwnProperty(key)) {
        var value = iter[key];
        this.render_nodes(f, {key: key, value: value});
      }}
    }
    return f;
  };
